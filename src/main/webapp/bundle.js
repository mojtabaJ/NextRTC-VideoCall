...
window.app = {
  ...
  createConversationA: function () {
    console.log("createConversationA");
    var convId = "A";
    nextRTC.create(convId);
  },
  ...
  joinConversationA: function () {
    console.log("joinConversationA");
    var convId = "A";
    nextRTC.join(convId);
  },
...
nextRTC.on('localStream', function (stream) {
  var dest = $("#templateB").clone().prop({id: 'local'});
  $("#my-video").append(dest);
  dest[0].srcObject = stream.stream;
  dest[0].muted = true;
});

nextRTC.on('remoteStream', function (stream) {
  var dest = $("#templateA").clone().addClass('remotestream').prop({id: stream.member});
  $("#user-video").append(dest);
  dest[0].srcObject = stream.stream;
});
